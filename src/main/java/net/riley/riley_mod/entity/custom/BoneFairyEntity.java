...
public class BoneFairyEntity extends TamableAnimal implements FlyingAnimal {
    ...
    private static final EntityDataAccessor<Integer> ATTACK_SIDE =
            SynchedEntityData.defineId(BoneFairyEntity.class, EntityDataSerializers.INT);
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(ATTACK_SIDE, 0); // 0 = Left, 1 = Right

    }

    public void setAttacking(boolean attacking) {
        this.entityData.set(ATTACKING, attacking);
        // Randomize side only on the server when the attack starts
        if (attacking && !this.level().isClientSide) {
            this.entityData.set(ATTACK_SIDE, this.random.nextInt(2));
        }
    }
    public int getAttackSide() {
        return this.entityData.get(ATTACK_SIDE);
    }
    ...
    private void setupAnimationStates() {
        if (this.isAttacking() && attackAnimationTimeout <= 0) {
            attackAnimationTimeout = 10; // Match your animation length (0.5s = 10 ticks)
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Animal.createLivingAttributes()
                ...
                .add(Attributes.MOVEMENT_SPEED, 0.2D) // Increased from 0.2
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand pHand) {
        ...
        if (itemstack.is(Items.SKELETON_SKULL) && !this.isBaby()) {
            ...
            if (!this.level().isClientSide) {
                SkeletonFairyEntity babySkeletonFairy = this.convertTo(RileyModEntities.SKELETONFAIRY.get(), true);

                if (babySkeletonFairy != null) {
                    // EXPLICIT TAME TRANSFER
                    if (this.isTame()) {
                        babySkeletonFairy.setTame(true);
                        babySkeletonFairy.setOwnerUUID(this.getOwnerUUID());
                        babySkeletonFairy.setOrderedToSit(this.isOrderedToSit());
                    }

                    babySkeletonFairy.setAge(-24000);
                    this.level().broadcastEntityEvent(babySkeletonFairy, (byte)7);
                }
            }else {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new MeleeAttackGoal(this, 1.2D, false) {
            ...
            public void start() {
                super.start();
                ((BoneFairyEntity)this.mob).setAttacking(true);
            }
            @Override
            public void stop() {
                super.stop();
                ((BoneFairyEntity)this.mob).setAttacking(false);
            }
        });

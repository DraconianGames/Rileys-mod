...
public class RileyModPortalItem extends Item {
    public RileyModPortalItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.isClientSide()) {
            // Try X axis first
            Optional<RileyModPortalShape> optional = RileyModPortalShape.findEmptyPortalShape(level, portalPos, Direction.Axis.X);

            // If X fails, try Z axis
            if (optional.isEmpty()) {
                optional = RileyModPortalShape.findEmptyPortalShape(level, portalPos, Direction.Axis.Z);
            }

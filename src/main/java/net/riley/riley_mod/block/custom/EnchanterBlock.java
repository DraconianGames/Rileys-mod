...
public class EnchanterBlock extends Block {
    ...
    public void setPlacedBy(Level pLevel, BlockPos pPos, BlockState pState, @Nullable LivingEntity pPlacer, ItemStack pStack) {
        ...
        if (!pLevel.isClientSide) {
            BlockState filler = RileyModBlocks.ENCHANTER_FILLER.get().defaultBlockState();
        ...
        if (!pState.is(pNewState.getBlock())) {
            ...
            for (int x = 0; x <= 0; x++) {
                for (int z = 0; z <= 0; z++) {
                    for (int y = 0; y <= 1; y++) {
                        BlockPos targetPos = pPos.offset(x, y, z);
                        if (!targetPos.equals(pPos) && pLevel.getBlockState(targetPos).is(RileyModBlocks.ENCHANTER_FILLER.get())) {

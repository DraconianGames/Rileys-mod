...
public class RileyModDimensions {
    ...
    public static void bootstrapStem(BootstapContext<LevelStem> context) {
        ...
        NoiseBasedChunkGenerator wrappedChunkGenerator = new NoiseBasedChunkGenerator(
                new FixedBiomeSource(biomeRegistry.getOrThrow(RileyModBiomes.ABYSS_FOREST_BIOME)),
                noiseGenSettings.getOrThrow(NoiseGeneratorSettings.OVERWORLD));

        NoiseBasedChunkGenerator noiseBasedChunkGenerator = new NoiseBasedChunkGenerator(
                MultiNoiseBiomeSource.createFromList(
                        new Climate.ParameterList<>(List.of(Pair.of(
                                        Climate.parameters(0.3F, 0.2F, 1F, 0F, 1F, -2F, 0.0F), biomeRegistry.getOrThrow(RileyModBiomes.ABYSS_FOREST_BIOME)),
                                Pair.of(
                                        Climate.parameters(0.4F, 0.2F, 1F, 0F, 1F, -2F, 0.0F), biomeRegistry.getOrThrow(RileyModBiomes.ABYSS_PLAINS))


                        ))),
                noiseGenSettings.getOrThrow(NoiseGeneratorSettings.OVERWORLD));

        LevelStem stem = new LevelStem(dimTypes.getOrThrow(RileyModDimensions.ABYSSDIM_TYPE), noiseBasedChunkGenerator);
